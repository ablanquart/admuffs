...
#if !ADMUFFS_HAVE_OPENSSL
    // Built without OpenSSL (macOS with no Homebrew): wss:// is unavailable.
    // Callers already fall back to the TVs' plaintext ports where they exist.
    (void)timeout_ms;
    LOG_WARN("ws: TLS (wss) not available in this build -- install OpenSSL "
             "(brew install openssl@3) and rebuild to talk to %s over TLS",
             host.c_str());
    return false;
#else
...
bool WsClient::recv_text(std::string& out, int timeout_ms) {
    ...
    for (;;) {
        ...
        // Cap the peer-declared frame size before allocating: a rogue/compromised
        // TV on the LAN could otherwise send a 2^64 length and force a bad_alloc.
        // Real SSAP/Tizen control messages are a few KB; 4 MiB is ample slack.
        if (len > (4u << 20)) return false;
