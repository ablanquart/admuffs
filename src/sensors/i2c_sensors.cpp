...
// I2C userspace access (/dev/i2c-*) is a Linux kernel interface; on other
// platforms the sensor slots simply report as unavailable (fail-soft, exactly
// like a Pi with I2C disabled).
namespace admuffs {
SensorReadings read_sensors(const std::string& bus) {
    (void)bus;
    SensorReadings r;
    r.i2c_present = false;
    r.i2c_error = "I2C sensors require Linux (/dev/i2c-1 on the Pi); "
