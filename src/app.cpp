...
void App::begin_suppression() {
    ...
    if (to_lower(cfg_.mute_mode) == "duck") {
        double before = room_tap() ? measure_room_db(2) : NAN;
        LOG_INFO("=> ducking TV volume (%d steps)", cfg_.duck_steps);
        for (int i = 0; i < cfg_.duck_steps; ++i) {
            tv_->send(TvCommand::VolumeDown);
            sleep_ms(150);
        }
        // With a room mic, measure how much the duck actually attenuated the
        // room and compensate the loudness detector so ducked audio is still
        // judged on the original scale (else the quiet we created reads as
        // "program resumed").
        if (room_tap() && loudness_ && !std::isnan(before)) {
            sleep_ms(2000);  // let post-duck audio fill the measurement window
            double after = measure_room_db(2);
            if (!std::isnan(after)) {
                double comp = std::clamp(before - after, 0.0, 20.0);
                loudness_->set_comp_db(comp);
                LOG_INFO("duck: measured %.1f dB attenuation; compensating", comp);
            }
        }
    } else {
        LOG_INFO("=> muting TV");
...
const SettingDef kSettings[] = {
    ...
    {"mute_mode", false},  {"duck_steps", true},  {"max_mute_s", true},
...
std::string App::config_json() {
    ...
    j["mute_mode"] = cfg_.mute_mode;       j["duck_steps"] = cfg_.duck_steps;
...
std::string App::config_set(const std::string& key, const std::string& value) {
    ...
    if (key == "method") {
        ...
        if (lv != "mute" && lv != "duck" && lv != "normalize") return reject("mute | duck | normalize");
        ...
        cfg_.mute_mode = lv;
    } else if (key == "duck_steps" || key == "max_mute_s" ||
               key == "norm_interval_ms" || key == "norm_max_range" ||
               key == "web_port") {
        if (!is_int(v)) return reject("whole number required");
        int n = atoi(v.c_str());
        if (key == "duck_steps") cfg_.duck_steps = n;
        else if (key == "max_mute_s") cfg_.max_mute_s = n;
        else if (key == "norm_interval_ms") cfg_.norm_interval_ms = n;
        else if (key == "norm_max_range") cfg_.norm_max_range = n;
        else cfg_.web_port = n;
    } else if (key == "norm_tolerance_db") {
        if (!is_num(v)) return reject("number required");
        cfg_.norm_tolerance_db = atof(v.c_str());
...
void App::set_automute(bool enabled) {
    ...
    // Mute/duck: keep the TV's state consistent when we're mid-commercial.
...
int App::run(const std::atomic<bool>& stop) {
    ...
    const bool muting_blinds_audio =
        room_tap() && !normalize_mode && to_lower(cfg_.mute_mode) != "duck";
    if (muting_blinds_audio && audio_bus_)
        LOG_WARN("audio_tap=room with mute_mode=mute: a room mic can't hear "
                 ...
                 "consider mute_mode=duck or normalize instead.",
                 cfg_.max_mute_s);
    if (normalize_mode) {
        if (cfg_.norm_target_db < -1.0)
            ...
            LOG_WARN("normalize mode: no target set -- press SET VOLUME TARGET "
    ...
    while (!stop.load() && !restart_requested_.load()) {
        ...
        for (auto& s : sources_) {
            ...
            // taps and duck mode (with compensation) keep hearing the
        ...
        if (automute_.load() && normalize_mode) {
            // Normalize mode: no mute/duck reaction -- the continuous leveler
    ...
    // Never leave the TV muted/ducked/shifted when admuffs exits.
