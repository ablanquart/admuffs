...
std::vector<std::string> list_recorded_keys() {
    ...
    } catch (...) { /* corrupt/absent file -> report none */ }
    return keys;
}
...
#else  // !__linux__: IR capture needs the Linux rc-core /dev/lirc* interface.

std::string find_rx_device() { return ""; }

RecordResult record_ir_key(const std::string& key) {
    RecordResult r;
    r.key = key;
    r.error = "IR recording requires Linux rc-core (/dev/lirc*) -- "
