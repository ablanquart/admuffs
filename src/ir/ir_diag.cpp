...
// The IR transceiver rides on Linux rc-core; on other platforms the check
// simply reports that and points at the supported setup.
int run_ir_check(const Config& cfg) {
    (void)cfg;
    printf("admuffs IR hardware check\n");
    printf("========================\n\n");
    printf("IR hardware support requires Linux (the /dev/lirc* rc-core\n");
    printf("interface used by the ANAVI Infrared pHAT on a Raspberry Pi).\n\n");
    printf("On this OS, admuffs still provides:\n");
    printf("  - network TV control (Roku / Samsung / LG / Sony / Vizio APIs)\n");
    printf("  - the web remote and settings hub\n");
    printf("  - the setup wizard (choose method=api, or ir_backend=dryrun)\n");
    return 1;
}
