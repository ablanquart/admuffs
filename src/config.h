...
struct Config {
    ...
    //   "volume_drop"      - lower the volume drop_steps steps instead; with a room
    ...
    //                 FIX VOLUME TARGET button while the TV is at the volume
    //                 you like.
    // Default is "volume_drop" because the default tap is a room mic, and a
    // volume drop is the only muting mode a mic-only setup can time correctly.
    std::string mute_mode = "volume_drop";
    int drop_steps = 6;
    ...
    // Failsafe: never stay muted/dropped longer than this without a positive
