...
std::string os_pretty_name() {
    std::string name = run_capture("sw_vers -productName 2>/dev/null");
    std::string ver  = run_capture("sw_vers -productVersion 2>/dev/null");
    if (name.empty()) return "macOS";
    return ver.empty() ? name : name + " " + ver;
}
...
std::string hardware_model() {
    ...
    if (sysctlbyname("hw.model", model, &len, nullptr, 0) == 0 && model[0])
        out = model;                                   // e.g. "Mac14,2"
    char chip[128] = {0};
    len = sizeof(chip) - 1;                            // e.g. "Apple M2"
    if (sysctlbyname("machdep.cpu.brand_string", chip, &len, nullptr, 0) == 0 &&
        chip[0])
        out += out.empty() ? chip : std::string(" (") + chip + ")";
    return out.empty() ? "Mac" : out;
}
...
std::string uptime_human() {
    ...
    if (sysctlbyname("kern.boottime", &boot, &len, nullptr, 0) == 0 &&
        boot.tv_sec > 0)
        s = (long)(time(nullptr) - boot.tv_sec);
    char buf[64];
...
#ifdef __APPLE__
    (void)unit;
    return "unsupported";   // systemd is Linux-only; run manually or via launchd
#else
...
#ifdef __APPLE__
    err = "service install is Linux-only (systemd). On macOS run "
          "'admuffs --run' manually, or wrap it in a launchd plist "
          "(~/Library/LaunchAgents) if you want it to start at login.";
    return false;
#else
...
#ifdef __APPLE__
    err = "service install/uninstall is Linux-only (systemd)";
    return false;
#else
