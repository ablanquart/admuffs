...
bool run_setup_wizard(Config& cfg, const IrDatabase& db, const std::string& config_path) {
    ...
    do {
        ...
        if (!cfg.audio_device.empty()) {
            ...
            // Volume drop and Normalize keep hearing (quieter) audio, so a mic can
            // time them correctly.
            ...
            modes.push_back(std::string("Volume drop: lower volume during ads; "
                                        "detection keeps hearing") +
                            (room ? "   (recommended)" : ""));
            ids.push_back("volume_drop");
            modes.push_back("Normalize: continuously level volume to a target "
                            "you set (web remote: FIX VOLUME TARGET)");
            ids.push_back("normalize");

            std::string cur = to_lower(cfg.mute_mode);
            if (room && cur == "mute") cur = "volume_drop";  // not offered for room mic
            int init = 0;
            ...
            if (mm >= 0) cfg.mute_mode = ids[mm];
                ...
                cfg.mute_mode = "volume_drop";  // cancelled, but never save room+mute
        }
