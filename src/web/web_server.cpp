...
<div class="remote">
  ...
  <div id="sampling" class="panel">
    ...
    <button class="menu-btn" id="s-level">SET VOLUME TARGET
  ...
  <div id="record" class="panel">
    ...
    <div class="panel-help">Point your TV's real remote at the pHAT's IR
      receiver. Recorded keys override the built-in database immediately.</div>
    <select id="reckey"></select>
    <button class="menu-btn" id="rec-go">&#9210; RECORD THIS KEY
...
<script>
  ...
  // ---- record remote codes ----
  const REC_KEYS = ['KEY_MUTE','KEY_POWER','KEY_VOLUMEUP','KEY_VOLUMEDOWN',
    'KEY_CHANNELUP','KEY_CHANNELDOWN','KEY_UP','KEY_DOWN','KEY_LEFT',
    'KEY_RIGHT','KEY_OK','KEY_BACK','KEY_MENU','KEY_HOME','KEY_SOURCE',
    'KEY_0','KEY_1','KEY_2','KEY_3','KEY_4','KEY_5','KEY_6','KEY_7',
    'KEY_8','KEY_9'];
  REC_KEYS.forEach(k => {
    const o = document.createElement('option');
    o.value = k; o.textContent = k.replace('KEY_', '').replace('SOURCE','INPUT');
    $('reckey').appendChild(o);
  });
  $('rec-go').addEventListener('click', () => {
    const k = $('reckey').value;
    $('recinfo').textContent = '⏺ waiting — press ' +
      k.replace('KEY_','') + ' on your remote NOW (9 s)…';
    fetch('/ir/record/' + k, { method: 'POST' }).then(r => r.json())
      .then(j => {
        flash($('rec-go'), j.ok);
        $('recinfo').textContent = j.ok
          ? '✓ ' + j.key + ' recorded (' + j.pulses +
            ' pulses via ' + j.device + ') — active immediately'
          : j.error;
      })
  ...
  const CFG_LABELS = { tv_brand:'TV brand', tv_model:'TV model',
    ...
    duck_steps:'duck steps', max_mute_s:'max mute (s)',
  ...
  // ---- info ----
  function infRow(k, v) {
    return '<div class="inf-row"><span class="k">' + k +
    ...
    fetch('/info').then(r => r.json()).then(j => {
      ...
      if (!j.i2c) {
        h += '<div class="panel-help">' +
             (j.i2c_error || 'I2C bus not available — enable it in raspi-config') +
  ...
  function loadSvcHint() {
    fetch('/info').then(r => r.json()).then(j => {
      $('svc-hint').textContent = (j.service === 'active')
...
void WebServer::loop() {
    while (running_) {
        ...
        // Recording blocks up to ~9 s; generous socket timeouts cover it.
...
void WebServer::handle_client(int fd) {
    ...
    if (method == "POST" && path == "/auth") {
        ...
        if (!hooks_.record_key) resp = not_wired();
            ...
            LOG_INFO("web remote: recording %s from the IR receiver...", key.c_str());
