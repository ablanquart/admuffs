...
Auth::Auth(Config& cfg, std::string config_path)
    ...
    if (cfg_.pin_salt.empty()) {
        ...
        if (cfg_.pin_salt.empty()) {
            // CSPRNG failure at first boot is effectively impossible, but if
            // it happens, say so loudly rather than silently degrading.
            LOG_ERROR("web auth: CSPRNG unavailable; using a fixed PIN salt -- "
                      "investigate this host's entropy source");
            cfg_.pin_salt = "admuffs-fallback-salt";
        }
    }
