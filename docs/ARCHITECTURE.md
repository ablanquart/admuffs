...
Mute/Unmute; `duck` steps the volume down/up by `duck_steps`.
...
- **Duck compensation** (`room` + `duck`): after stepping the volume down,
  the app measures the actual room attenuation (RMS before vs. after) and
  installs it via `LoudnessSource::set_comp_db()`, so ducked audio is judged
