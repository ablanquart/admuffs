...
> **Renamed:** this project was previously called *admute*. Upgrading an
> existing install? Migrate your config and clean up the old binary:
>
> ```bash
> mv ~/.config/admute ~/.config/admuffs 2>/dev/null
> mv ~/.config/admuffs/admute.conf ~/.config/admuffs/admuffs.conf 2>/dev/null
> sudo rm -f /usr/local/bin/admute
> sudo rm -rf /usr/local/share/admute     # after 'sudo cmake --install build'
> ```
>
> (An un-migrated IR database at the old share path is still found as a
> legacy fallback, and Samsung/LG/Vizio TVs may show a fresh pairing prompt
> since the client now identifies as "admuffs".)
